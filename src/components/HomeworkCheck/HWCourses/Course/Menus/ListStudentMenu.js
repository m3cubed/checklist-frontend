...
import { handleDeleteHWStudent } from "../../../../../actions/HomeworkCheck/hwStudents";
//Redux
...
class ListStudentMenu extends Component {
	...
	handleDelete = () => {
		this.setState(
			...
			() => {
				this.props.dispatch(handleDeleteHWStudent(this.props.target));
				this.props.toggle();
			}
		);
	};
		...
		return (
			<Fragment>
				<Dialog open={this.state.openDialog} onClose={this.toggleDialog}>
					<DialogContent>
						<DialogContentText>
							Are you sure you want to delete this student? All the data
							attached to them will be deleted.
						</DialogContentText>
					</DialogContent>
					<DialogActions>
